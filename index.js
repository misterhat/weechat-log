...
var ACTIONS = {
    ...
    TIME_MATCH = /^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})/,
    ACTION_MATCH = /^(\*|<--|-->|--)/,
    NICK_MATCH = /([@|~|&|%|+]?\S+)/,
...
module.exports.fromStream = function (source) {
    var parsed = map({ objectMode: true}, function (line) {
        return parseLine(line.toString());
    });

    source = byline.createStream(source);
